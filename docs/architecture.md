...
Counting policy belongs to the workspace being measured. It is an immutable `Config` built once per run from `defaults.json` plus the layers above and passed to the functions that count, including the worker processes; nothing reads it from module-level state.
