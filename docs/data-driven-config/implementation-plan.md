...
- Produces:
  ...
  - `build_config(defaults: dict, layers: dict, report_title: str) -> Config` (today's overlay semantics) and `default_config(report_title: str = "SourceTempo") -> Config` (packaged defaults only). `load_config` arrives in PR 3, when per-layer validation needs to know each layer's source file.
